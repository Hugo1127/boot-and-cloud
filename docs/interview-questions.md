...
| 生命周期 | 容器管理 | 获取时创建 |
| 循环依赖 | 支持（三级缓存） | 不支持 |
| 并发安全 | 需要线程安全设计 | 天然线程安全 |
| 适用场景 | 无状态服务 | 有状态对象 |

**原理延伸**：
- 本项目实现：DefaultListableBeanFactory.doGetBean()
- Singleton 使用 per-bean-lock 保证并发安全
- Prototype 不缓存，每次 createBean()

***

### 面试题 28：Setter 注入和构造器注入的区别？

**标准应答**：

**构造器注入**：依赖通过构造函数参数传入，保证 Bean 创建时依赖已就绪，适合强依赖。
**Setter 注入**：依赖通过 setter 方法注入，支持可选依赖，适合可选配置。
**字段注入**：代码简洁，但不符合依赖注入最佳实践。

**最佳实践**：优先构造器注入，可选依赖用 Setter 注入。
...
### 面试题 29：虚拟线程 vs 传统线程的区别？

**标准应答**：

虚拟线程（JDK 21+，本项目使用 JDK 25）是轻量级线程，由 JVM 调度而非 OS 调度。

**性能数据**（本项目实测，JDK 25）：

```
高并发 100000 任务：
  虚拟线程：105ms (952,381 任务/秒)
  传统线程：1461ms (68,446 任务/秒)
  性能提升：92.81%
...
**结论**：虚拟线程适合 IO 密集型场景，CPU 密集型场景传统线程更优。

***

## 总结

本文档整理了 Boot&Cloud 框架涉及的**29 道高频面试题**，涵盖：
